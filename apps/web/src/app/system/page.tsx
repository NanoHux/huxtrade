...
export default async function Page(){
  const [status,preferences,health]=await Promise.all([
    api<Record<string,unknown>>("/api/settings/status",{coinglassMode:"disabled",coinglassReady:false,telegramConfigured:false,variationalMode:"disabled",liveTradingEnabled:false}),
  ...
  return <><Header eyebrow="Operations & Safety" title="系统设置与健康"/><div className="pageGrid">
    ...
    <section className="panel"><div className="panelHead"><h2>永久错误记录</h2><small>最近 100 条 · 数据库不自动清理</small></div><table><thead><tr><th>发生时间</th><th>服务</th><th>币种</th><th>代码</th><th>错误</th><th>交易</th></tr></thead><tbody>{health.errors.map((error)=><tr key={String(error.id)}><td>{time(error.occurred_at)}</td><td>{String(error.service)}</td><td>{String(error.asset_code??"—")}</td><td className="mono">{String(error.code)}</td><td className="subtle">{String(error.message)}</td><td className={error.blocks_trading?"down":"up"}>{error.blocks_trading?"阻断":"不阻断"}</td></tr>)}{!health.errors.length&&<tr><td colSpan={6} className="empty">尚无业务错误记录</td></tr>}</tbody></table></section>
