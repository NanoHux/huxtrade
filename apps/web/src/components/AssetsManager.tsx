...
export function AssetsManager({initial}:{initial:Asset[]}){
  ...
  // Assets that carry trading history cannot be deleted — that would take
  // their orders and signals with them — so retired tickers stay in the table
  // for ever. Defaulting the view to the ones actually trading keeps the list
  // about the live book instead of about everything ever tried.
  const [showDisabled,setShowDisabled]=useState(false);
  const visible=assets.filter((asset)=>showDisabled?!asset.tradeEnabled:asset.tradeEnabled);
  const disabledCount=assets.filter((asset)=>!asset.tradeEnabled).length;
  ...
  async function submit(event:React.FormEvent){
    ...
    async function toggleTrade(asset:Asset){
    const response=await fetch(`${publicApi}/api/assets/${asset.id}`,{method:"PATCH",headers:{"content-type":"application/json"},
      body:JSON.stringify({code:asset.code,binanceSymbol:asset.binanceSymbol,coinglassUrl:asset.coinglassUrl,variationalUrl:asset.variationalUrl,
        collectEnabled:asset.collectEnabled,signalEnabled:asset.signalEnabled,tradeEnabled:!asset.tradeEnabled})});
    const body=await response.json();
    if(!response.ok){setMessage(`保存失败：${body.message??body.error}`);return;}
    setAssets(assets.map((item)=>item.id===asset.id?body:item));
    setMessage(`${asset.code} 交易已${asset.tradeEnabled?"停止":"开启"}`);
  }
  ...
  return <div className="pageGrid">
    ...
    <section className="panel"><div className="panelHead"><div><h2>白名单 · {assets.length}/120</h2><small>{showDisabled?"正在查看已停止交易的币种":"仅显示交易中的币种"}　·　数据异常后只允许手动恢复</small></div><button className="btn" onClick={()=>setShowDisabled(!showDisabled)}>{showDisabled?`← 交易中 (${assets.length-disabledCount})`:`已停止交易 (${disabledCount}) →`}</button></div><table><thead><tr><th>代码</th><th>数据映射</th><th>采集/信号/交易</th><th>连接 / 状态</th><th>最后更新</th><th>原因</th><th>操作</th></tr></thead><tbody>{visible.map((asset)=><tr key={asset.id}><td><b>{asset.code}</b></td><td className="mono">{asset.binanceSymbol}<br/><span className="subtle">CG · {asset.coinglassSymbol}</span></td><td>{[asset.collectEnabled,asset.signalEnabled,asset.tradeEnabled].map((value)=>value?"●":"○").join("  ")}</td><td><span className={`tag ${asset.connectionStatus==="ERROR"?"red":asset.connectionStatus!=="CONNECTED"?"orange":""}`}>{asset.connectionStatus??"PENDING"}</span><br/><span className={`tag ${asset.paused?"red":""}`}>{asset.paused?"PAUSED":"ACTIVE"}</span></td><td className="subtle">{asset.lastUpdatedAt?new Date(asset.lastUpdatedAt).toLocaleString("zh-CN",{timeZone:"Asia/Shanghai"}):"—"}</td><td className="subtle">{asset.pauseReason??"—"}</td><td><div className="toolbar"><button className="btn" onClick={()=>edit(asset)}>编辑</button><button className={asset.paused?"btn primary":"btn"} onClick={()=>pause(asset)}>{asset.paused?"手动恢复":"暂停"}</button><button className={asset.tradeEnabled?"btn":"btn primary"} onClick={()=>toggleTrade(asset)}>{asset.tradeEnabled?"停止交易":"开启交易"}</button><button className="btn danger" onClick={()=>remove(asset)}>删除</button></div></td></tr>)}{!visible.length&&<tr><td colSpan={7} className="empty">{showDisabled?"没有已停止交易的币种":"没有交易中的币种"}</td></tr>}</tbody></table></section>
  </div>;
}
