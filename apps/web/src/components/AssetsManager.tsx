...
const emptyForm={code:"",binanceSymbol:"",coinglassSymbol:"",variationalUrl:"",collectEnabled:true,signalEnabled:true,tradeEnabled:false};
...
export function AssetsManager({initial}:{initial:Asset[]}){
  ...
  function edit(asset:Asset){setEditingId(asset.id);setForm({code:asset.code,binanceSymbol:asset.binanceSymbol,coinglassSymbol:asset.coinglassSymbol,variationalUrl:asset.variationalUrl,collectEnabled:asset.collectEnabled,signalEnabled:asset.signalEnabled,tradeEnabled:asset.tradeEnabled});setMessage("正在编辑，保存时会重新校验变更的数据映射");}
  function reset(){setEditingId(null);setForm(emptyForm);}
  async function submit(event:React.FormEvent){
    event.preventDefault();setMessage("正在校验 Binance 与 CoinGlass…");
    ...
    setAssets(editingId?assets.map((asset)=>asset.id===editingId?body:asset):[...assets,body]);setMessage("已校验并保存");reset();
  }
  ...
  return <div className="pageGrid">
    <section className="panel"><div className="panelHead"><div><h2>{editingId?"编辑白名单币种":"新增白名单币种"}</h2><small>保存前同时验证 Binance USDT 永续与 CoinGlass Binance Heatmap</small></div>{editingId&&<button className="btn" onClick={reset}>取消编辑</button>}</div>
      <form onSubmit={submit} className="formGrid"><div className="field"><label>内部代码</label><input required value={form.code} onChange={(event)=>{const code=event.target.value.toUpperCase();setForm({...form,code,binanceSymbol:`${code}USDT`,coinglassSymbol:`${code}USDT`});}} placeholder="BTC"/></div><div className="field"><label>Binance Symbol · 自动识别</label><input required readOnly value={form.binanceSymbol} placeholder="BTCUSDT"/></div><div className="field"><label>CoinGlass Symbol · 自动识别</label><input required readOnly value={form.coinglassSymbol} placeholder="BTCUSDT"/></div><div className="field"><label>Variational 市场 URL</label><input required type="url" value={form.variationalUrl} onChange={(event)=>setForm({...form,variationalUrl:event.target.value})} placeholder="https://..."/></div><div className="checkboxes"><label><input type="checkbox" checked={form.collectEnabled} onChange={(event)=>setForm({...form,collectEnabled:event.target.checked})}/> 采集</label><label><input type="checkbox" checked={form.signalEnabled} onChange={(event)=>setForm({...form,signalEnabled:event.target.checked})}/> 信号</label><label><input type="checkbox" checked={form.tradeEnabled} onChange={(event)=>setForm({...form,tradeEnabled:event.target.checked})}/> 交易</label></div><div className="toolbar"><button className="btn primary">{editingId?"校验并更新":"自动识别、校验并保存"}</button><span className="subtle">{message}</span></div></form>
    </section>
