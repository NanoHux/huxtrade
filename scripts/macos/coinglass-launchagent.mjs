...
const label="io.huxtrade.coinglass-agent";
...
const command=`cd ${shell(projectRoot)} && exec corepack pnpm --filter @huxtrade/coinglass-agent start`;
...
<plist version="1.0"><dict>
  ...
  <key>StandardOutPath</key><string>${xml(join(logDirectory,"coinglass-agent.out.log"))}</string>
  <key>StandardErrorPath</key><string>${xml(join(logDirectory,"coinglass-agent.err.log"))}</string>
</dict></plist>
`;
...
async function assertSafeInstallConfig(){
  ...
  const cdp=setting("COINGLASS_CDP_URL");
  if(setting("COINGLASS_ADAPTER_MODE")!=="browser")throw new Error("Set COINGLASS_ADAPTER_MODE=browser before installing launchd");
  if(cdp&&!/^http:\/\/(127\.0\.0\.1|localhost|\[::1\]):\d+$/.test(cdp))throw new Error("COINGLASS_CDP_URL must be a loopback Chrome debugging port");
}
...
}else throw new Error("Usage: coinglass-launchagent.mjs install|status|uninstall|render");
